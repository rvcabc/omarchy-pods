...
void BluetoothMonitor::onPropertiesChanged(const QDBusMessage &message)
{
    ...
    // A UUIDs or ServicesResolved change on an already connected device is the moment the AAP service becomes reachable.
...
void BluetoothMonitor::onInterfacesAdded(const QDBusMessage &message)
{
    ...
    // A foreign pair in pairing mode also advertises the AAP UUID; only a bonded pair is ours to connect to.
    if (deviceProps.value("Connected").toBool() && deviceProps.value("Paired").toBool()) {
        const QString macAddress = deviceProps.value("Address").toString();
        const QString deviceName = deviceProps.value("Name").toString();
        emit deviceConnected(macAddress, deviceName);
        LOG_DEBUG("AirPods device added and connected: " << macAddress << " Name: " << deviceName);
    }
}
