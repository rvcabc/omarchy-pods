...
public:
    ...
    void saveToSettings(QSettings &settings)
    {
        ...
        // User preferences previously lost across daemon restarts —
        // persist them alongside the other settings.
        settings.setValue("conversationalAwareness", conversationalAwareness());
        settings.setValue("oneBudANCMode", oneBudANCMode());
        settings.setValue("adaptiveNoiseLevel", adaptiveNoiseLevel());
        settings.endGroup();
    }
    void loadFromSettings(const QSettings &settings)
    {
        ...
        // wins.
        const QString persistedModelNumber = settings.value("DeviceInfo/modelNumber", "").toString();
        setModelNumber(persistedModelNumber);
        if (!persistedModelNumber.isEmpty()) {
            // Re-run parseModelNumber so newly-added map entries take
            // effect on the next launch (the enum int in QSettings
            // would otherwise stick at whatever the previous build
            // resolved to — Unknown for then-uncovered devices).
            setModel(parseModelNumber(persistedModelNumber));
        } else {
        ...
        setConversationalAwareness(settings.value("DeviceInfo/conversationalAwareness", false).toBool());
        setOneBudANCMode(settings.value("DeviceInfo/oneBudANCMode", false).toBool());
        setAdaptiveNoiseLevel(settings.value("DeviceInfo/adaptiveNoiseLevel", 50).toInt());
    }
